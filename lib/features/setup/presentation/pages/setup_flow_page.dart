...
class _SetupFlowPageState extends ConsumerState<SetupFlowPage> {
  ...
  bool get _canContinue => switch (_step) {
        ...
        6 => _goal != null,
        7 => _provider != null,
        8 => _apiKeyController.text.trim().isNotEmpty,
        9 => _baseUrlController.text.trim().isNotEmpty,
        10 => _modelController.text.trim().isNotEmpty,
        _ => false,
      };

  String get _buttonLabel => _step == _lastStep ? 'C\'est parti !' : 'Continuer';
              ...
              Expanded(
                child: PageView(
                  ...
                  children: [
                    ...
                    _NameStep(controller: _nameController, onChanged: () => setState(() {}), onSubmit: _canContinue ? _next : null),
                    _SexStep(selected: _sex, onSelect: (s) { setState(() => _sex = s); _next(); }),
                    _AgeStep(age: _age, onChanged: (v) => setState(() => _age = v)),
                    _MeasurementsStep(weight: _weight, height: _height, onWeightChanged: (v) => setState(() => _weight = v), onHeightChanged: (v) => setState(() => _height = v)),
                    _ActivityStep(selected: _activity, onSelect: (a) { setState(() => _activity = a); _next(); }),
                    _GoalStep(selected: _goal, onSelect: (g) { setState(() => _goal = g); _next(); }),
                    _ProviderStep(selected: _provider, onSelect: (p) { setState(() { _provider = p; _baseUrlController.clear(); _modelController.clear(); }); _next(); }),
                    _ApiKeyStep(provider: _provider, controller: _apiKeyController, showKey: _showApiKey, onToggleShow: () => setState(() => _showApiKey = !_showApiKey), onChanged: () => setState(() {})),
                    _BaseUrlStep(provider: _provider, controller: _baseUrlController, onChanged: () => setState(() {})),
                    _ModelStep(provider: _provider, controller: _modelController, onChanged: () => setState(() {})),
                  ],
                ),
              ),
              if (_step > 0)
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: const Text('Commencer', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w600)),
            ),
...
class _NameStep extends StatelessWidget {
  const _NameStep({required this.controller, required this.onChanged, this.onSubmit});
            ...
            child: Padding(
              ...
              child: GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child: Column(
                    children: [
                      Text(sex == BiologicalSex.male ? '👨' : '👩', style: const TextStyle(fontSize: 44)),
    ...
    return _StepBody(
      ...
      child: Column(
        children: [
          ...
          _StyledSlider(value: age, min: 10, max: 100, divisions: 90, onChanged: onChanged),
          const SizedBox(height: 8),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('10 ans', style: Theme.of(context).textTheme.labelSmall?.copyWith(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.4))),
              Text('100 ans', style: Theme.of(context).textTheme.labelSmall?.copyWith(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.4))),
            ],
          ),
        ],
      ),
...
class _MeasurementsStep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _StepBody(
      ...
      child: Column(
        children: [
          _SliderRow(label: 'Poids', value: weight, unit: 'kg', min: 30, max: 200, divisions: 340, onChanged: onWeightChanged),
          const SizedBox(height: 28),
          _SliderRow(label: 'Taille', value: height, unit: 'cm', min: 130, max: 230, divisions: 200, onChanged: onHeightChanged),
        ],
      ),
    );
  }
    ...
    return Column(
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(label, style: Theme.of(context).textTheme.labelLarge?.copyWith(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.6))),
            Text(
              '${value.round()} $unit',
        ...
        const SizedBox(height: 8),
        _StyledSlider(value: value, min: min, max: max, divisions: divisions, onChanged: onChanged),
      ],
    );
  }
}
              ...
              children: [
                AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    color: isSelected ? AppColors.accent : Colors.transparent,
                    border: Border.all(color: isSelected ? AppColors.accent : Theme.of(context).colorScheme.outline, width: 2),
                  ),
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(level.label, style: TextStyle(fontWeight: FontWeight.w600, color: isSelected ? AppColors.accent : Theme.of(context).colorScheme.onSurface)),
                      Text(level.description, style: Theme.of(context).textTheme.labelSmall?.copyWith(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.5))),
                    ],
                  ),
                ),
              ],
...
class _GoalStep extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const emojis = {GoalType.lose: '📉', GoalType.maintain: '⚖️', GoalType.gain: '💪'};
    return _StepBody(
      emoji: '🎯',
      title: 'Ton objectif ?',
...
class _ProviderStep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final providers = [
      (AiProvider.openai, '🟢', 'OpenAI', 'Modèles GPT — le plus courant', 'platform.openai.com/api-keys'),
      (AiProvider.gemini, '🔵', 'Google Gemini', 'Gemini Flash, Pro...', 'aistudio.google.com/app/apikey'),
      (AiProvider.anthropic, '🟣', 'Anthropic Claude', 'Claude Opus, Sonnet...', 'console.anthropic.com/settings/keys'),
    ];
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(name, style: TextStyle(fontWeight: FontWeight.w700, color: isSelected ? AppColors.accent : Theme.of(context).colorScheme.onSurface)),
                      Text(desc, style: Theme.of(context).textTheme.labelSmall?.copyWith(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.55))),
                      Text('Clé sur : $site', style: Theme.of(context).textTheme.labelSmall?.copyWith(color: AppColors.accent.withValues(alpha: 0.7), fontSize: 10)),
                    ],
                  ),
                ),
                if (isSelected) const Icon(Icons.check_circle_rounded, color: AppColors.accent),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: IconButton(
                icon: Icon(showKey ? Icons.visibility_off_rounded : Icons.visibility_rounded, size: 20),
...
class _BaseUrlStep extends StatelessWidget {
  const _BaseUrlStep({required this.provider, required this.controller, required this.onChanged});
  final AiProvider? provider;
  final TextEditingController controller;
  final VoidCallback onChanged;
...
class _ModelStep extends StatelessWidget {
  const _ModelStep({required this.provider, required this.controller, required this.onChanged});
  final AiProvider? provider;
  final TextEditingController controller;
  final VoidCallback onChanged;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? AppColors.accent.withValues(alpha: 0.07) : Theme.of(context).colorScheme.surfaceContainerHighest,
...
class _InfoTile extends StatelessWidget {
  const _InfoTile({required this.text, this.icon = Icons.lightbulb_outline_rounded});
  final String text;
  final IconData icon;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(icon, size: 15, color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.45)),
